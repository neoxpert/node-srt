...
const srtRepoPath = env.NODE_SRT_LOCAL_REPO
  ? `file://${path.join(__dirname, env.NODE_SRT_LOCAL_REPO)}`
  : SRT_REPO;
...
function build() {
  console.log(
    "Building SRT SDK and prerequisites for current platform:",
    process.platform,
  );
  let opensslRoot;
...
function buildWin32() {
  ...
  const openssl = spawnSync(
    "vcpkg",
    ["install", "openssl", "--triplet", `${process.arch}-windows`],
    { cwd: process.env.VCPKG_ROOT, shell: true },
  );
  if (openssl.stdout) console.log(openssl.stdout.toString());
  if (openssl.status) {
  ...
  const pthreads = spawnSync(
    "vcpkg",
    ["install", "pthreads", "--triplet", `${process.arch}-windows`],
    { cwd: process.env.VCPKG_ROOT, shell: true },
  );
  if (pthreads.stdout) console.log(pthreads.stdout.toString());
  if (pthreads.status) {
  ...
  const integrate = spawnSync("vcpkg", ["integrate", "install"], {
    cwd: process.env.VCPKG_ROOT,
    shell: true,
  });
  if (integrate.stdout) console.log(integrate.stdout.toString());
  if (integrate.status) {
  ...
  if (generator.stdout) console.log(generator.stdout.toString());
  if (generator.status) {
  ...
  if (build.stdout) console.log(build.stdout.toString());
  if (build.status) {
...
function buildNx() {
  ...
  const configure = spawnSync("./configure", ["--prefix", buildDir], {
    cwd: srtSourcePath,
    shell: true,
    stdio: "inherit",
  });
  ...
  const make = spawnSync("make", [`-j${numCpus}`], {
    cwd: srtSourcePath,
    shell: true,
    stdio: "inherit",
  });
